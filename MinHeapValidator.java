public class MinHeapValidator implements BTValidator{
    ...
    private boolean isMinHeap(IBinTree aTree ){
        if(aTree.isEmpty()) {
            return true;
        }
        else {
            boolean left = this.lessThanAll(aTree.getRoot(), aTree.getLeft());
            boolean right = this.lessThanAll(aTree.getRoot(), aTree.getRight());
            return left && right ;}
    }

    private boolean lessThanAll(int elt, IBinTree aTree){
        if(aTree.isEmpty()){
            return true;
        }
        else{
            return elt < aTree.getRoot() &&
                    this.lessThanAll(elt, aTree.getLeft()) && this.lessThanAll(elt, aTree.getRight());
        }
    }
